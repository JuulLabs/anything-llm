...
- Allocates the agent (if new) so it gets its own `ssh_port`/`app_port`/`vnc_port`/`relay_port`,
  writes `agents/mytask/.env`, starts the whitelist proxy (shared) and **this agent's own**
  LLM relay process, boots the VM.
...
## Give an agent MCP tools (optional)
...
```
- **Fully opt-in.** Omit `--mcp-url` and nothing changes: no MCP port is allocated,
  no `10.0.2.102` pinhole is opened, no MCP relay process starts, and no guest MCP
  config is written. Existing agents/launches that don't pass it are completely
  unaffected.
- When `--mcp-url` **is** given, this agent gets its own `mcp_port` (persisted in
  `agents/mytask/agent.json`, allocated from `MCP_PORT_BASE` the same way
  `relay_port` is), its own `host/mcp-relay.mjs` process (pidfile
  `agents/mytask/mcp-relay.pid`, log `agents/mytask/mcp-relay.log`), and its own
  `10.0.2.102` guestfwd pinhole — all independent per agent, same isolation model
  as the LLM relay (so parallel agents can point at different MCP servers/tokens
  with no cross-talk).
...
- `mcp_port` is only allocated at `create` time (when `--mcp-url` is first passed
  for a brand-new agent). An agent created without it can't retroactively get one
  from `launch-task.sh` — either destroy/recreate it, or start it manually with
  `./open-computer up mytask --mcp-port <port>`.
...
```bash
./open-computer down mytask      # graceful stop (shared-folder output stays on host); also stops this agent's LLM relay and MCP relay (if any)
./open-computer destroy mytask   # delete VM + agent dir (shared folder included — copy out first!); also stops its LLM relay and MCP relay (if any)
```
...
- MCP relaying (`host/mcp-relay.mjs`) follows the exact same per-agent isolation model as the LLM relay, but `mcp_port` is OPTIONAL on `AgentJson` and only ever allocated when an agent opts in via `--mcp-url`/`create --mcp` — unlike `relay_port`, most agents never get one, and that's the expected/backward-compatible state.
